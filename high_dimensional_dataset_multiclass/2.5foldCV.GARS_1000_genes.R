...
diff_time <- c()
feats_range <- 15:25
control <- rfeControl(functions=rfFuncs, method="boot",number=1)
predictionlist_TR <- list()
predictionlist_Val <- list()
predictionlist_TS <- list()
nome <- "rf"
predictionlist_TR[[nome]] <- list(predictions=list(),labels=list())
predictionlist_Val[[nome]] <- list(predictions=list(),labels=list())
predictionlist_TS[[nome]] <- list(predictions=list(),labels=list())
...
for (crval in 1:5){
  ...
  for (j in seq_len(length(res))){
    max_final_fit[j] <- round(max(FitScore(res[[j]])), 2)
  }
  max_final_fit
  
  best_set <- which(max_final_fit == max(max_final_fit))[1]
  data_reduced_GARS <- MatrixFeatures(res[[best_set]])
  data_reduced_GARS <- as.data.frame(data_reduced_GARS)
  ...
  # Training Set
  trainingSet_DM <- cbind(data_reduced_GARS,class.TR[,2,drop=FALSE])
  varNames <- colnames(data_reduced_GARS)
